"""
Soft Actor-Critic (SAC) for diffusion policy training (without EBM).

This module implements pure SAC training without energy-based model integration,
...
class TrainSACDiffusionAgent(TrainAgent):
    """
    Pure SAC training agent without EBM integration.
    
    This class implements standard SAC training for ablation studies.
    """
    ...
    def __init__(self, cfg):
        ...
        log.info(f"Initialized pure SAC agent with buffer_size={self.buffer_size}, batch_size={self.batch_size}")
        ...
        while self.itr < self.n_train_itr:
            ...
            if self.itr % self.log_freq == 0:
                ...
                if eval_mode:
                    ...
                    log.info(
                        f"{self.itr}: step {cnt_train_step:8d} | reward {avg_episode_reward:8.4f} | t:{time:8.4f}"
                    )
                    if self.use_wandb:
                        wandb.log({
