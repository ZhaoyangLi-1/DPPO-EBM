...
# ------------------------------------------------------------------
# 6.  Wrapper class for config compatibility
# ------------------------------------------------------------------
class EBMWrapper(nn.Module):
    """Wrapper for EBM class to provide a simpler interface."""
    
    def __init__(self, obs_dim, action_dim, hidden_dim=256, num_layers=3, **kwargs):
        super().__init__()
        
        # Create a config object with the nested ebm structure
        class Config:
            def __init__(self):
    ...
    def state_dict(self):
        ...
        try:
            ...
            import logging
            logging.warning(f"EBMWrapper state_dict() failed with error: {e}")
